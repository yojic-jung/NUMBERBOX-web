...
//적분, 시그마 위아래 다이렉트 이동
const directMoveToAboveDom = [
  { focusDom: 'nbSigmaSumSub', targetDom: 'nbSigmaSumSup' },
  { focusDom: 'nbSigmaSumSign', targetDom: 'nbSigmaSumSup' },
  { focusDom: 'nbIntSub', targetDom: 'nbIntSup' },
  { focusDom: 'nbDoubleIntSub', targetDom: 'nbDoubleIntSup' },
  { focusDom: 'nbTripleIntSub', targetDom: 'nbTripleIntSup' },
];
const directMoveToBelowDom = [
  { focusDom: 'nbSigmaSumSup', targetDom: 'nbSigmaSumSub' },
  { focusDom: 'nbSigmaSumSign', targetDom: 'nbSigmaSumSub' },
  { focusDom: 'nbIntSup', targetDom: 'nbIntSub' },
  { focusDom: 'nbDoubleIntSup', targetDom: 'nbDoubleIntSub' },
  { focusDom: 'nbTripleIntSup', targetDom: 'nbTripleIntSub' },
];
...
const allSelDom = ['nbExpTmp', 'nbSubTmp', 'nbOverDotBase', 'nbAccentBase', 'nbRootBase', 'nbOverlineBase', 'nbRightArrowBase', 'nbArcBase', 'nbBiDirArrowBase'];
...
export const reg_convertNotTransferdNbBox = async (domId) => {
  ...
  for (let i = 0; i < nbBrck.length; i++) {
    if (
      nbBrck[i].querySelectorAll('.nbFracInDenom, .nbFracInNumer, .nbIntegralInDenom, .nbIntegralInNumer, .nbSigmaSumInDenom, .nbSigmaSumInNumer, .nbLimInDenom, .nbLimInNumer')
        .length !== 0
    ) {
  ...
  if (window.getSelection().getRangeAt(0).commonAncestorContainer.classList !== undefined) {
    if (window.getSelection().getRangeAt(0).commonAncestorContainer.classList.contains('nbBox')) {
      ...
      if (nbBoxes.length !== 0) {
        ...
        while (
          lastNbBox.parentElement !== null &&
          lastNbBox.parentElement.closest('.nbBox') !== null &&
          window.getSelection().containsNode(lastNbBox.parentElement.closest('.nbBox'))
        ) {
        ...
        while (
          firstNbBox.parentElement !== null &&
          firstNbBox.parentElement.closest('.nbBox') !== null &&
          window.getSelection().containsNode(firstNbBox.parentElement.closest('.nbBox'))
        ) {
...
export const reg_makeUndoRedoByCtrlKey = async (evType) => {
  ...
  if (window.getSelection().isCollapsed) {
    ...
    //셀렉트 된 상태라면 캐럿 앞 뒤로 추가 후 원래의 셀렉트 상태로 원복
    undoCollapsed = false;
    if (
      window.getSelection().getRangeAt(0).startContainer === window.getSelection().focusNode &&
...
export const reg_formulaTapMoveEv = async (event) => {
  ...
  if (!event.shiftKey && event.altKey && event.keyCode === 39) {
    //오른쪽으로 이동
  ...
  } else if (!event.shiftKey && event.altKey && event.keyCode === 37) {
    //왼쪽으로 이동
    event.preventDefault();
    for (let i = 0; i < formulaTap.length; i++) {
      if (formulaTap[i].classList.contains('selectedTab')) {
  ...
  for (let i = strtContainerChildren.length - 1; i >= 0; i--) {
    if (strtContainerChildren[i].nodeName === '#text') {
      for (let len = 0; len < 5; len++) {
        //텍스트 노드의 길이가 1부터 4인 것 까지 값이 모두 빈 텍스트 체크하고 삭제
...
export const reg_preventKeyEvent = async (event, isMyContents) => {
  ...
  //ctrl+z 구현
  if (!(event.ctrlKey && userKeyCode === 90) && event.keyCode !== 229 && userKeyCode !== 37 && userKeyCode !== 38 && userKeyCode !== 39 && userKeyCode !== 40) {
  ...
  //ctrl+y구현
  if (!(event.altKey && event.shiftKey) && userKeyCode === 89 && event.ctrlKey && userKeyCode !== 37 && userKeyCode !== 38 && userKeyCode !== 39 && userKeyCode !== 40) {
    event.preventDefault();
  ...
  if (
    (window.getSelection().anchorNode.tagName !== undefined && window.getSelection().anchorNode.tagName === 'TBODY') ||
  ...
  if ((userKeyCode === 38 || userKeyCode === 40) && window.getSelection().isCollapsed && !event.altKey) {
    ...
    if (isExeptDom) {
      ...
      if (focusParDom !== undefined) {
        if (userKeyCode === 38) {
          for (let i = 0; i < noApplyUpKeyList.length; i++) {
            if (focusParDom.closest('.' + noApplyUpKeyList[i]) !== null) {
              focusParDom = focusParDom.closest('.' + noApplyUpKeyList[i]);
            }
          }
        } else {
          for (let i = 0; i < noApplyDownKeyList.length; i++) {
            if (focusParDom.closest('.' + noApplyDownKeyList[i]) !== null) {
              focusParDom = focusParDom.closest('.' + noApplyDownKeyList[i]);
            }
          }
        }
      ...
      while (
        position.y < moveRange.getBoundingClientRect().y + moveRange.getBoundingClientRect().height &&
        position.y > moveRange.getBoundingClientRect().y - moveRange.getBoundingClientRect().height
      ) {
        document.getElementsByClassName('tmpCaretPoint')[0].remove();
        if (userKeyCode === 38) {
      ...
      if (moveStrtContainer.closest('#' + document.activeElement.id) !== null) {
        ...
        if (userKeyCode === 38) {
          if (document.activeElement.firstChild.classList === undefined) {
            ...
            //div 요소가 있을때 div 요소 밖에 집어넣으면 밑에 줄에 캐럿이 생성되어 커서 포인터가 정확하지 않음
    ...
    if (window.getSelection().getRangeAt(0).getBoundingClientRect().y - 10 < document.getElementById(document.activeElement.id).getBoundingClientRect().y) {
      document.getElementById(document.activeElement.id).scrollTop +=
        window.getSelection().getRangeAt(0).getBoundingClientRect().y - document.getElementById(document.activeElement.id).getBoundingClientRect().y - 10;
    } else if (
      window.getSelection().getRangeAt(0).getBoundingClientRect().y + 10 >
      document.getElementById(document.activeElement.id).getBoundingClientRect().y + document.getElementById(document.activeElement.id).getBoundingClientRect().height
    ) {
      document.getElementById(document.activeElement.id).scrollTop +=
        window.getSelection().getRangeAt(0).getBoundingClientRect().y +
        window.getSelection().getRangeAt(0).getBoundingClientRect().height -
        document.getElementById(document.activeElement.id).getBoundingClientRect().y -
        document.getElementById(document.activeElement.id).getBoundingClientRect().height +
        10;
    }
    window.getSelection().collapseToStart(); //collapseToStart 명령어 실행 안하면 셀렉트 상태라 옐로우 박스 안 입혀짐.
    document.getElementsByClassName('tmpCaretPoint')[0].remove();
  ...
  if (userKeyCode === 13) {
    ...
    if (position.x === 0 && position.y === 0) {
      ...
      if (
        (document.getElementsByClassName('tmpPositionDetectCaret')[0].previousSibling !== null &&
          document.getElementsByClassName('tmpPositionDetectCaret')[0].previousSibling.classList !== undefined &&
          document.getElementsByClassName('tmpPositionDetectCaret')[0].previousSibling.classList.contains('nbBox')) ||
        (document.getElementsByClassName('tmpPositionDetectCaret')[0].nextSibling !== null &&
          document.getElementsByClassName('tmpPositionDetectCaret')[0].nextSibling.classList !== undefined &&
          document.getElementsByClassName('tmpPositionDetectCaret')[0].nextSibling.classList.contains('nbBox'))
      ) {
  ...
  if (
    (userKeyCode === 37 || userKeyCode === 39 || userKeyCode === 8) &&
    document.activeElement.querySelectorAll('.editInnerTable').length !== 0 &&
    ...
    //표 오른쪽에서 백스페이스 시 표 전체 선택
      ...
      userKeyCode === 8 &&
      tmpNode.previousSibling !== null &&
      tmpNode.previousSibling.nodeName === 'TABLE' &&
      tmpNode.previousSibling.classList.contains('editInnerTable')
    ) {
  ...
  if (event.altKey) {
    ...
    if (mappingKey != null && !isWriteDisableDom) {
      //alt 단축키 사용한 경우
  ...
  setTimeout(function () {
    ...
    if (userKeyCode === 13 && willExecuteFormBlock) {
      ...
      while (tmpEnterBugCaret.length > 0) {
        ...
        if (tmpEnterBugCaret[0].parentElement !== null && tmpEnterBugCaret[0].parentElement !== undefined) {
          if (
            tmpEnterBugCaret[0].parentElement.nodeName === 'DIV' &&
            tmpEnterBugCaret[0].parentElement.childNodes.length === 1 &&
            tmpEnterBugCaret[0].parentElement.childNodes[0].classList.contains('tmpEnterBugCaret')
          ) {
    ...
    if (
      ...
      if (tmpUndoHtml.innerHTML !== document.activeElement.innerHTML && event.keyCode !== 229) {
        ...
        if (
          window.getSelection().getRangeAt(0).startContainer === window.getSelection().anchorNode &&
        ...
        currentData.activeId = document.activeElement.id; //현재 입력창 id
        currentData.innerHTML = undoHTML;
        if (previouseKeyCode[0] === 32 || previouseKeyCode[0] === 13)
          currentData.isSpaceOrEnter = true; //space 또는 enter 여부
    ...
    } else if (!(event.altKey && event.shiftKey) && userKeyCode === 90 && event.ctrlKey) {
      //ctrl+z 실행
      if (undoArr.length > 0) {
        //데이터가 있는 경우에만 실행
        for (let i = undoArr.length - 1; i >= 0; i--) {
          if (undoArr[i].activeId === document.activeElement.id) {
            //현재 입력창의 마지막 undo 데이터 가져오기
            document.activeElement.innerHTML = undoArr[i].innerHTML;
            if (undoArr[i].isCollapsed) {
              //포커스 및 셀렉트 셋팅
              window.getSelection().setBaseAndExtent(document.getElementsByClassName('tmpUndoCarot')[0], 0, document.getElementsByClassName('tmpUndoCarot')[0], 0);
              window.getSelection().collapseToStart();
              ...
              if (undoArr[i].rangeDirection === 'right') {
                if (undoArr[i].positionByHangulInput !== undefined) {
                  ...
                  window.getSelection().setBaseAndExtent(document.getElementsByClassName('tmpUndoCarot')[0], 1, document.getElementsByClassName('tmpUndoCarotEnd')[0], 0);
                }
              } else {
                window.getSelection().setBaseAndExtent(document.getElementsByClassName('tmpUndoCarotEnd')[0], 1, document.getElementsByClassName('tmpUndoCarot')[0], 0);
              }
              if (undoArr[i].positionByHangulInput === undefined) {
                //캐럿 제거, 캐럿 남아있으면 안됨.
                document.getElementsByClassName('tmpUndoCarot')[0].remove();
    ...
    if (!event.altKey && userKeyCode === 89 && event.ctrlKey) {
      if (redoArr.length > 0) {
        ...
        for (let i = redoArr.length - 1; i >= 0; i--) {
          if (redoArr[i].activeId === document.activeElement.id) {
            //현재 입력창의 마지막 undo 데이터 가져오기
            document.activeElement.innerHTML = redoArr[i].innerHTML;
            if (redoArr[i].isCollapsed) {
              //포커스 및 셀렉트 셋팅
              window.getSelection().setBaseAndExtent(document.getElementsByClassName('tmpUndoCarot')[0], 0, document.getElementsByClassName('tmpUndoCarot')[0], 0);
              window.getSelection().collapseToStart();
              document.getElementsByClassName('tmpUndoCarot')[0].remove();
...
export const reg_allSelFormulaRuleEnd = async (isMouseUp) => {
  ...
  //분수, 루트 분수, 경우의수, 이항계수는  하나의 수식요소 밑 같은 요소끼리 select시 전체 선택 안함.(다른요소일때는 전체선택)
    ...
    endConOneDepth.classList.contains('nbFracBox') ||
    endConOneDepth.classList.contains('nbRootFracBox') ||
    endConOneDepth.classList.contains('nbCaseBrckBox') ||
    endConOneDepth.classList.contains('nbThrCasekBox') ||
...
export const reg_allSelFormulaFocusRule = async (isMouseUp) => {
  ...
  //분수, 루트 분수, 경우의수, 이항계수는  하나의 수식요소 밑 같은 요소끼리 select시 전체 선택 안함.(다른요소일때는 전체선택)
    ...
    focusNbBox.classList.contains('nbFracBox') ||
    focusNbBox.classList.contains('nbRootFracBox') ||
    focusNbBox.classList.contains('nbCaseBrckBox') ||
    focusNbBox.classList.contains('nbThrCasekBox') ||
  ...
  if (anchorNbBorderBox !== null && focusNbBorderBox !== null) {
    ...
    if (anchorNbBorderBox === focusNbBorderBox) {
      for (let i = 0; i < allSelDom.length; i++) {
        ...
        if (anchorNbBorderBox.classList.contains(allSelDom[i])) {
          ...
          if (
            ...
            if (
              document.getSelection().getRangeAt(0).endContainer.nodeName === '#text' &&
      ...
      if (rootAnchorNbBox === rootFocusNbBox) {
        ...
        for (let i = 0; i < allSelDom2.length; i++) {
          //루트 수식 뿐만 아니라 걸리는 모든 수식에 대해 체크 해야함
        ...
        if (isExceptCase) {
          ...
          for (let i = 0; i < allSelDom2[boxTypeIdx].borderType.length; i++) {
            ...
            innerWhile: while (focusNbBorderBoxForExcept.parentElement.closest('.borderBox') !== null || focusNbBorderBoxForExcept.parentElement.closest('.writeDisable')) {
              if (focusNbBorderBoxForExcept.parentElement.closest('.borderBox') === null)
                focusNbBorderBoxForExcept = focusNbBorderBoxForExcept.parentElement.closest('.writeDisable');
            ...
            innerWhile2: while (
              anchorNbBorderBoxForExcept.parentElement.closest('.borderBox') !== null ||
              anchorNbBorderBoxForExcept.parentElement.closest('.writeDisable') !== null
            ) {
...
export const reg_newSelectFormulaElementForKeyEv = async (event) => {
  ...
  if (window.getSelection().isCollapsed) {
    ...
    for (let i = cusUnderLine.length - 1; i >= 0; i--) {
      if (cusUnderLine[i].innerHTML.charCodeAt(0) === 65279) {
        //밑줄 버튼 여러번 누르고 키 입력 안 한 경우  cusUnderLine태그 제거
...
export const reg_selectCheck = () => {
  ...
  if (!window.getSelection().isCollapsed) {
    window
      .getSelection()
      .setBaseAndExtent(window.getSelection().anchorNode, window.getSelection().anchorOffset, window.getSelection().anchorNode, window.getSelection().anchorOffset);
  }
};
  ...
  if (anchorInnerTbTd !== focusInnerTbTd) {
    ...
    for (let row = lowerRowCol.substring(0, 1); row <= biggerRowCol.substring(0, 1); row++) {
      for (let col = lowerRowCol.substring(1); col <= biggerRowCol.substring(1); col++) {
        ...
        focusInnerTbTd
          .closest('.editInnerTable')
          .querySelector('#' + innerTbTdId)
          .classList.add('nbSelectionTbTd');
      }
    }
...
export const reg_nbComplie = async (event) => {
  ...
  if (event.keyCode === 86) {
    //event.ctrlKey && event.keyCode === 86로 진행하면 ctrl키를 더 늦게 누른 경우 적용 안됨
    ...
    for (let i = 0; i < nbAbsBrckBox.length; i++) {
      if (
        nbAbsBrckBox[i].querySelectorAll(
          '.nb-Abs-BrckBase .nbFracBox, .nb-Abs-BrckBase .nbIntegralBox, .nb-Abs-BrckBase .nbDoubleIntegralBox, .nb-Abs-BrckBase .nbTripleIntegralBox, .nb-Abs-BrckBase .nbSigmaSumBox, .nb-Abs-BrckBase .nbIntBrckBox'
        ).length !== 0
      ) {
    ...
    for (let i = 0; i < nbAbsBrckBox.length; i++) {
      if (
        nbAbsBrckBox[i].querySelectorAll(
          '.nb-Abs-BrckBase .nbFracInDenom, .nb-Abs-BrckBase .nbFracInNumer, .nb-Abs-BrckBase .nbIntegralInDenom, .nb-Abs-BrckBase .nbIntegralInNumer, .nb-Abs-BrckBase .nbSigmaSumInDenom, .nb-Abs-BrckBase .nbSigmaSumInNumer, .nb-Abs-BrckBase .nbLimInDenom, .nb-Abs-BrckBase .nbLimInNumer'
        ).length !== 0
      ) {
        nbAbsBrckBox[i].classList.add('nbConvert');
    ...
    for (let i = 0; i < nbRoundBrckBox.length; i++) {
      if (
        nbRoundBrckBox[i].querySelectorAll(
          '.nb-R-BrckBox .nbFracBox, .nb-R-BrckBase .nbIntegralBox, .nb-R-BrckBase .nbDoubleIntegralBox, .nb-R-BrckBase .nbTripleIntegralBox, .nb-R-BrckBase .nbSigmaSumBox, .nb-R-BrckBase .nbIntBrckBox'
        ).length !== 0
      ) {
    ...
    for (let i = 0; i < nbRoundBrckBox.length; i++) {
      if (
        nbRoundBrckBox[i].querySelectorAll(
          '.nb-R-BrckBase .nbFracInDenom, .nb-R-BrckBase .nbFracInNumer, .nb-R-BrckBase .nbIntegralInDenom, .nb-R-BrckBase .nbIntegralInNumer, .nb-R-BrckBase .nbSigmaSumInDenom, .nb-R-BrckBase .nbSigmaSumInNumer, .nb-R-BrckBase .nbLimInDenom, .nb-R-BrckBase .nbLimInNumer'
        ).length !== 0
      ) {
        nbRoundBrckBox[i].classList.add('nbConvert');
    ...
    for (let i = 0; i < nbCBrckBox.length; i++) {
      if (
        nbCBrckBox[i].querySelectorAll(
          '.nb-C-BrckBox .nbFracBox, .nb-C-BrckBase .nbIntegralBox, .nb-C-BrckBase .nbDoubleIntegralBox, .nb-C-BrckBase .nbTripleIntegralBox, .nb-C-BrckBase .nbSigmaSumBox, .nb-C-BrckBase .nbIntBrckBox'
        ).length !== 0
      ) {
    ...
    for (let i = 0; i < nbCBrckBox.length; i++) {
      if (
        nbCBrckBox[i].querySelectorAll(
          '.nb-C-BrckBase .nbFracInDenom, .nb-C-BrckBase .nbFracInNumer, .nb-C-BrckBase .nbIntegralInDenom, .nb-C-BrckBase .nbIntegralInNumer, .nb-C-BrckBase .nbSigmaSumInDenom, .nb-C-BrckBase .nbSigmaSumInNumer, .nb-C-BrckBase .nbLimInDenom, .nb-C-BrckBase .nbLimInNumer'
        ).length !== 0
      ) {
        nbCBrckBox[i].classList.add('nbConvert');
    ...
    for (let i = 0; i < nbSBrckBox.length; i++) {
      if (
        nbSBrckBox[i].querySelectorAll(
          '.nb-S-BrckBox .nbFracBox, .nb-S-BrckBase .nbIntegralBox, .nb-S-BrckBase .nbDoubleIntegralBox, .nb-S-BrckBase .nbTripleIntegralBox, .nb-S-BrckBase .nbSigmaSumBox, .nb-S-BrckBase .nbIntBrckBox'
        ).length !== 0
      ) {
    ...
    for (let i = 0; i < nbSBrckBox.length; i++) {
      if (
        nbSBrckBox[i].querySelectorAll(
          '.nb-S-BrckBase .nbFracInDenom, .nb-S-BrckBase .nbFracInNumer, .nb-S-BrckBase .nbIntegralInDenom, .nb-S-BrckBase .nbIntegralInNumer, .nb-S-BrckBase .nbSigmaSumInDenom, .nb-S-BrckBase .nbSigmaSumInNumer, .nb-S-BrckBase .nbLimInDenom, .nb-S-BrckBase .nbLimInNumer'
        ).length !== 0
      ) {
        nbSBrckBox[i].classList.add('nbConvert');
    ...
    for (let i = 0; i < nbFracLineConvert.length; i++) {
      ...
      if (children !== null) {
        if (
          children.querySelectorAll('.nbRootBox').length !== 0 ||
    ...
    for (let i = 0; i < nbFracLineConvert2.length; i++) {
      ...
      if (children !== null) {
        if (children.querySelectorAll('.nbOverlineBox').length !== 0 || children.querySelectorAll('.nbArrowBox').length !== 0) {
...
*/
  ...
  for (let i = 0; i < nbFracInFracBoxes.length; i++) {
    if (nbFracInFracBoxes[i].querySelectorAll('.nbDenom .nbFracBox').length === 0 && nbFracInFracBoxes[i].querySelectorAll('.nbNumer .nbFracBox').length === 0) {
  ...
  for (let i = 0; i < nbBrckInFracBoxes.length; i++) {
    ...
    if (nbBrckInFracBoxes[i].classList.contains('nb-Abs-BrckBox')) {
      if (
        nbBrckInFracBoxes[i].querySelectorAll(
          '.nb-Abs-BrckBase .nbFracBox, .nb-Abs-BrckBase .nbIntegralBox, .nb-Abs-BrckBase .nbDoubleIntegralBox, .nb-Abs-BrckBase .nbTripleIntegralBox, .nb-Abs-BrckBase .nbIntBrckBox, .nb-Abs-BrckBase .nbSigmaSumBox'
        ).length === 0
      ) {
    ...
    if (nbBrckInFracBoxes[i].classList.contains('nb-R-BrckBox')) {
      if (
        nbBrckInFracBoxes[i].querySelectorAll(
          '.nb-R-BrckBase .nbFracBox, .nb-R-BrckBase .nbIntegralBox, .nb-R-BrckBase .nbDoubleIntegralBox, .nb-R-BrckBase .nbTripleIntegralBox, .nb-R-BrckBase .nbIntBrckBox, .nb-R-BrckBase .nbSigmaSumBox'
        ).length === 0
      ) {
        nbBrckInFracBoxes[i].classList.remove('nbConvert');
        nbBrckInFracBoxes[i].classList.remove('nbBrckInFrac');
    ...
    if (nbBrckInFracBoxes[i].classList.contains('nb-C-BrckBox')) {
      if (
        nbBrckInFracBoxes[i].querySelectorAll(
          '.nb-C-BrckBase .nbFracBox, .nb-C-BrckBase .nbIntegralBox, .nb-C-BrckBase .nbDoubleIntegralBox, .nb-C-BrckBase .nbTripleIntegralBox, .nb-C-BrckBase .nbIntBrckBox, .nb-C-BrckBase .nbSigmaSumBox'
        ).length === 0
      ) {
        nbBrckInFracBoxes[i].classList.remove('nbConvert');
        nbBrckInFracBoxes[i].classList.remove('nbBrckInFrac');
    ...
    if (nbBrckInFracBoxes[i].classList.contains('nb-S-BrckBox')) {
      if (
        nbBrckInFracBoxes[i].querySelectorAll(
          '.nb-S-BrckBase .nbFracBox, .nb-S-BrckBase .nbIntegralBox, .nb-S-BrckBase .nbDoubleIntegralBox, .nb-S-BrckBase .nbTripleIntegralBox, .nb-S-BrckBase .nbIntBrckBox, .nb-S-BrckBase .nbSigmaSumBox'
        ).length === 0
      ) {
        nbBrckInFracBoxes[i].classList.remove('nbConvert');
        nbBrckInFracBoxes[i].classList.remove('nbBrckInFrac');
  ...
  for (let i = 0; i < nbBrckInFracInFracBoxes.length; i++) {
    ...
    if (nbBrckInFracInFracBoxes[i].classList.contains('nb-Abs-BrckBox')) {
      if (
        nbBrckInFracInFracBoxes[i].querySelectorAll(
          '.nb-Abs-BrckBase .nbFracInDenom, .nb-Abs-BrckBase .nbFracInNumer, .nb-Abs-BrckBase .nbIntegralInDenom, .nb-Abs-BrckBase .nbIntegralInNumer, .nb-Abs-BrckBase .nbSigmaSumInDenom, .nb-Abs-BrckBase .nbSigmaSumInNumer, .nb-Abs-BrckBase .nbLimInDenom, .nb-Abs-BrckBase .nbLimInNumer'
        ).length === 0
      ) {
  ...
  for (let i = 0; i < nbFracLineConvert.length; i++) {
    if (
      nbFracLineConvert[i].querySelectorAll('.nbDenom .nbRootBox').length === 0 &&
  ...
  for (let i = 0; i < nbFracLineConvert2.length; i++) {
    if (nbFracLineConvert2[i].querySelectorAll('.nbDenom .nbOverlineBox').length === 0 && nbFracLineConvert2[i].querySelectorAll('.nbDenom .nbArrowBox').length === 0) {
...
export const reg_undoStackByClick = async (activeId) => {
  ...
  if (window.getSelection().anchorNode !== null) {
    //표 추가의 경우 activeElement안에 포커스가 없는 상태일 수 있음
    if (
      window.getSelection().getRangeAt(0).startContainer === window.getSelection().anchorNode &&
      window.getSelection().getRangeAt(0).startOffset === window.getSelection().anchorOffset
    ) {
...
export const reg_enableImageResizeInDiv = (id, isMultiMode) => {
  ...
  window.addEventListener('mousedown', function (e) {
    ...
    if (
      !(
        e.target.classList.contains('resize-frame') ||
        e.target.classList.contains('imgWidthHeight') ||
        e.target.classList.contains('imgLeftAlign') ||
        e.target.classList.contains('imgCenterAlign') ||
        e.target.classList.contains('imgRightAlign') ||
        e.target.classList.contains('imgFloatLeft') ||
        e.target.classList.contains('imgFloatRight') ||
        e.target.classList.contains('imgAlignCenterLikeChar')
      )
    ) {
      reset();
