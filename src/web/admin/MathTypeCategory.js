...
const MathTypeCategory = () => {
  ...
  const [mathTypePopupContents, setMathTypePopupContents] = useState(
    new Array()
  );
  ...
  const unitFoldClickFunction = (event, btnWrapClassName, foldBtnClassName) => {
    let typeBtnWrap =
      event.target.parentElement.querySelectorAll(btnWrapClassName);
  ...
  const subTypeChangeFunction = async (event) => {
    ...
    let jsonObj = await nb_dataFetch(
      '/mathInfo/takeConCntByUnitUniqNo?unitUniqNo=' +
        event.target.dataset.unitUniqNo,
      true
    );
    jsonObj = jsonObj.cntList;

    document.getElementById('mathTypeChngPopupDiv').classList.remove('hide');
    document.getElementById('mathTypeAddBtn').dataset.unitUniqNo =
      event.target.dataset.unitUniqNo;

    let typeBtn = event.target
      .closest('.thrUnitBtnWrap')
      .querySelectorAll('.typeBtn');
    document.getElementById('mathTypeChngPopupTitle').innerHTML = event.target
      .closest('.thrUnitBtnWrap')
      .querySelector('.thrUnitBtn').innerHTML;
    ...
    for (let i = 0; i < typeBtn.length; i++) {
      ...
      for (let j = 0; j < jsonObj.length; j++) {
        if (
          parseInt(typeBtn[i].dataset.unitUniqNo) === jsonObj[j].unitUniqNo &&
          parseInt(typeBtn[i].dataset.typeNo) === jsonObj[j].typeNo
        ) {
  ...
  const unitListSetFunction = (subjectList, secUnitList, thrUnitList) => {
    for (let i = 0; i < subjectList.length; i++) {
      ...
      tmpSpanFoldBtn.addEventListener('click', (event) =>
        unitFoldClickFunction(event, '.secUnitBtnWrap', '.subjectFoldBtn')
      );
    ...
    for (let i = 0; i < subjectBtnList.length; i++) {
      for (let j = 0; j < secUnitList.length; j++) {
        if (
          ...
          tmpSpanFoldBtn.addEventListener('click', (event) =>
            unitFoldClickFunction(event, '.thrUnitBtnWrap', '.secUnitFoldBtn')
          );
          tmpDiv.append(tmpSpanFoldBtn);
          let tmpSpan = document.createElement('span');
    ...
    for (let i = 0; i < secUnitBtnList.length; i++) {
      for (let j = 0; j < thrUnitList.length; j++) {
        if (
          ...
          tmpSpanFoldBtn.addEventListener('click', (event) =>
            unitFoldClickFunction(event, '.typeBtnWrap', '.thrUnitFoldBtn')
          );
          tmpDiv.append(tmpSpanFoldBtn);
        ...
        if (
          ...
          if (event.target.dataset.typeExist === 'false') {
            ...
            for (let j = 0; j < thrUnitBtnWrap.length; j++) {
              let typeBtnWrap =
                thrUnitBtnWrap[j].querySelectorAll('.typeBtnWrap');
    ...
    if (subjectInfo.mainVal.includes('중등')) {
      ...
      return (
        <span
          ...
          dangerouslySetInnerHTML={{ __html: subjectInfo.mainVal }}
        ></span>
      );
    }
  });
  ...
  const mathTypeDel = async (event, conCnt) => {
    ...
    let inputVal = prompt(
      "삭제를 진행하실 경우 '삭제'라고 따옴표 없이 입력해주세요.",
      ''
    );
    if (inputVal !== '삭제') {
    ...
    let jsonObj = await nb_dataFetch(
      '/mathInfo/typeDel?unitUniqNo=' +
        event.target.dataset.unitUniqNo +
        '&typeNo=' +
        event.target.dataset.typeNo,
      2000
    );
  ...
  const mathContentsMoveSel = async (event) => {
    ...
    event.target
      .closest('.typeChngBtnRootWrap')
      .querySelector('.conMoveCnclBtnWrap')
      .classList.remove('hide');

    let contentsMoveToBtn =
      document.getElementsByClassName('contentsMoveToBtn');
    ...
    event.target
      .closest('.mathTypeChngPopupContentsTB')
      .querySelector('.contentsMoveToBtn')
      .classList.add('from');
    event.target
      .closest('.mathTypeChngPopupContentsTB')
      .querySelector('.contentsMoveToBtn').innerHTML = 'FROM';
  };

  const mathContentsMoveTo = async (event) => {
    if (event.target.classList.contains('from')) {
      await nb_fadeInOutB(
        '이동하려는 문제의 유형과 같은 유형을 선택할 수 없습니다.',
        2500
      );
      return false;
    }
  ...
  const mathTypeChange = async (event, mathTypeId) => {
    ...
    event.target
      .closest('.typeChngBtnRootWrap')
      .querySelector('.typeConChngBtnWrap')
      .classList.remove('hide');
    ...
    window
      .getSelection()
      .selectAllChildren(document.getElementById(mathTypeId));
  ...
  const mathContentsMoveApply = async () => {
    ...
    let inputVal = prompt(
      '"FROM"으로 선택하신 유형에 속해 있는 문제를 "TO"로 선택하신 유형으로 이동 시키겠습니까?\n이동시키려면 "예"라고 따옴표 없이 입력해주세요.'
    );

    if (inputVal === '예') {
      let returnObj = await nb_dataFetch(
        '/mathInfo/contentsMoveFromTo?fromUnitUniqNo=' +
          fromUnitUniqNo +
          '&fromTypeNo=' +
          fromTypeNo +
          '&toUnitUniqNo=' +
      ...
      if (returnObj.isSuccess) {
        ...
        document
          .getElementById('mathTypeAddPopupBtn')
        ...
        await nb_fadeInOutB(
          '문제 이동에 실패하였습니다. 다시 시도해주세요.',
          2500
        );
      }
    } else {
      await nb_fadeInOutB(
        '잘못 입력하였습니다.\n문제 이동을 원할 경우 다시 입력해주세요.',
        2500
      );
    }
  };
  ...
  const mathTypeChngInit = async (event) => {
    ...
    for (let i = 0; i < typeBtn.length; i++) {
      if (
        typeBtn[i].dataset.unitUniqNo === unitUniqNo &&
        typeBtn[i].dataset.typeNo === typeNo
      ) {
        document.getElementById(
          'mathType-' + unitUniqNo + '-' + typeNo
        ).innerHTML = typeBtn[i].innerHTML;
      }
    }
  };
  ...
  const mathTypeChangeCncl = async () => {
    ...
    let typeConChngBtnWrap =
      document.getElementsByClassName('typeConChngBtnWrap');
    for (let i = 0; i < typeConChngBtnWrap.length; i++) {
      typeConChngBtnWrap[i].classList.add('hide');
    }

    let conMoveCnclBtnWrap =
      document.getElementsByClassName('conMoveCnclBtnWrap');
  ...
  const mathTypeOrderChng = async () => {
    let mathType = document.getElementsByClassName(
      'mathTypeChngPopupContentsTB'
    );
    let formData = new FormData();
    for (let i = 0; i < mathType.length; i++) {
      let mathTypeArr = mathType[i].dataset.id.split('-');
      formData.append(
        'mathTypeInfoModel[' + i + '].unitUniqNo',
        mathTypeArr[0]
      );
      ...
      <div id='mathTypeChngPopupDiv' className='blindBox hide'>
        <div className='mathTypeChngPopupDiv'>
          <div
            ...
            onClick={() => {
              document
                .getElementById('mathTypeChngPopupDiv')
                .classList.add('hide');
            }}
          >
            X
          ...
          <div
            id='mathTypeChngPopupTitle'
            className='mathTypeChngPopupTitle'
          ></div>
          ...
          <ReactSortable
            id='mathTypeChngPopupContents'
            list={mathTypePopupContents}
            animation={200}
            setList={setMathTypePopupContents}
          >
            {mathTypePopupContents.map((contents, idx) => {
              let keyValue = contents.unitUniqNo + '-' + contents.typeNo;
              let mathTypeId =
                'mathType-' + contents.unitUniqNo + '-' + contents.typeNo;
              let mathConCntId =
                'mathConCnt-' + contents.unitUniqNo + '-' + contents.typeNo;
              return (
      ...
      <div id='mathTypeAddRootDiv' className='blindBox hide'>
        <div id='mathTypeAddDiv' className='mathTypeAddDiv'>
          <div
            ...
            onClick={() => {
              document
                .getElementById('mathTypeAddRootDiv')
                .classList.add('hide');
            }}
          >
            X
