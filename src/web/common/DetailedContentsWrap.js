    ...
    const likeContents = async (event, isBasedParent)=>{
        ...
        if(event.target.classList.contains("active") || event.target.classList.contains("active2")){
            ...
            if(isModalBase){
                ...
                document.getElementById("contentsLike"+contentsNo).classList.remove("active2");
            } 
    ...
    const putInMyRepo = async (event, isBasedParent)=>{
        ...
        if(event.target.classList.contains("active") || event.target.classList.contains("active2")){
            ...
            if(isModalBase){
                ...
                document.getElementById("contentsRepo"+contentsNo).classList.remove("active2");
            }
...
return (
    <div id="detailedConDiv" className="blindBox hide">
                    <div className='detailedConDiv '>
                        <div className='closeBtn' onClick={()=>{document.getElementById("detailedConDiv").classList.add("hide")}}>X</div>
                        <LicenseUi2 />
                                                        ...
                                                        <div>
                                                            <span id="likeRepoWrap">
                                                                <span className='userSearchBtn'>
                                                                    <span id="detailedContentsRepo" className="putRepoBtn"  onClick={(event)=>{putInMyRepo(event) }}></span>
                                                                    <span className='putRepoToolTip'>나의 저장소에 저장되었습니다</span>
                                                                </span>
                                                                <span className='userSearchBtn'>
                                                                    <span id="detailedContentsLike" className="likeBtn" onClick={(event)=>{likeContents(event);}}></span>
                                                                </span>
                                                            </span>
                                                            <span id="nicknamewrap" className='userSearchBtn' onClick={(event)=>{goToUserProfile(event)}}>
