...
import { nb_dataFetch, nb_fadeInOutB, nb_deleteRequest } from 'js/common/common_nb.js';
        ...
        <div key={idx}>
          <table className='followListTable'>
            <tbody>
              <tr>
                <td>
                  <Link className='linkNoneCss' to={'/userProfile?userNo=' + followUser.id}>
                    <img alt='프로필이미지' src={profileImgPath} className='contentsListProfile' />
                  </Link>
                </td>
                <td>
                  <Link className='linkNoneCss' to={'/userProfile?userNo=' + followUser.id}>
                    <span className='hoverUnderLine'>{followUser.nickname}</span>
                  </Link>
                </td>
                <td className='alignRight'>
                  <span
                    className='miniRedBtn'
                    data-user-no={followUser.id}
                    onClick={(event) => {
                      showFollowCancelpop(event);
                    }}>
                    팔로잉
                  </span>
                </td>
  ...
  return (
    <div id='followListBox' className='promptBoxScreen hide'>
      <div className='followList'>
        <div
          className='mini-closeBtn'
          onClick={() => {
            document.getElementById('followListBox').classList.add('hide');
          }}>
          X
        </div>
      ...
      <div id='myFollowCancelScreen' className='promptBoxScreen hide'>
        <div className='followCancelDiv'>
          ...
          <div className='marginTenAuto'>
            <span id='myFollowingNickname'></span>님의 팔로우를 취소하시겠어요?
          </div>
          <div
            id='myFollowCncl'
