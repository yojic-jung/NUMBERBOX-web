...
const ProfileComponent = ({isMine, userNo})=>{
    ...
    useEffect(()=>{
        const asyncUseEffect = async function(){
            ...
            if(userNo!==undefined && userNo !== ""){
                jsonObj = await nb_dataFetch('/takeUserProfile?userNo='+Number(userNo), true);
                if(jsonObj.isSuccess){
                    ...
                    document.getElementById("followerCnt").innerText = jsonObj.followerCnt;
                    
                }
            }else{
                jsonObj = await nb_dataFetch('/takeProfile', true);
                if(jsonObj.isSuccess){
                    setNickname(jsonObj.profile.nickname);
                    if(jsonObj.profile.profileImgPath !== null && jsonObj.profile.profileImgName !== null){
                        setImgPath(process.env.REACT_APP_SERVER_STATIC_HOST+jsonObj.profile.profileImgPath+jsonObj.profile.profileImgName)
                ...
                setMyFollower(jsonObj.myFollower)
                document.getElementById("myFollowerCnt").innerText = jsonObj.followerCnt;
                document.getElementById("myFollowingCnt").innerText = jsonObj.followingCnt;
            }
    ...
    const nicknameChange = async(event)=>{
        ...
        if(!regex.test(nickval)){
            ...
            let jsonObj = await nb_dataFetch('/changeNickname?nickname='+nickval, true);
            if(jsonObj.isSuccess){
    ...
    const followingUser = async(userNo)=>{
        let returnObj = await nb_dataFetch('/followingUser?userNo='+userNo, true);
        if(returnObj.isSuccess){
            document.getElementById("followingUser").classList.add("hide");
            document.getElementById("followedUser").classList.remove("hide");
            document.getElementById("followerCnt").innerText = returnObj.followerCnt;
        }else{
    ...
    return (
                            ...
                            <td>
                                <div>
                                    ...
                                    <div id="myNickNameWrap" className='relative'>
                                        <span id="myNickName" className='myNickName'>{nickname}</span>
                                        {isMine ?
                                        <>
                                            <span id="nicknameChngBtn" className='nicknameChngBtn' onClick={()=>{document.getElementById("myNickNameWrap").classList.add("hide");document.getElementById("nickChngWrap").classList.remove("hide");document.getElementById("nickname").focus()}}></span>
                                            <sup className='nicknameChngToolTip'>닉네임 수정하기</sup>
                                        </>
                                        : 
                                        <>
                                            <span id="followingUser" className='blueBoxBtn maginLTwoZero' onClick={()=>{followingUser(userNo)}}>팔로우</span>
                                            <span id="followedUser" className='followedUser hide' onClick={()=>{document.getElementById("followCancelScreen").classList.remove("hide")}}></span>
                                        </>
                                        }
                                    </div>
                                    <div id="nickChngWrap" className='nickChngWrap hide'>
                                        ...
                                        <span id="nickChngOkBtn" className='nickChngOkBtn' onClick={()=>{nicknameChange()}}>변경</span>
                                        <span id="nickChngCancelBtn" className='nickChngCancelBtn' onClick={()=>{document.getElementById("myNickNameWrap").classList.remove("hide");document.getElementById("nickChngWrap").classList.add("hide");}}>취소</span>
                                    </div>
                                </div>
                                ...
                                {isMine ?
                                    <>
                                        <div>
                                            <span id="myFollowingInfo" className='followWrap' onClick={()=>{showMyFollowList(true)}}>
                                                팔로잉 <span id="myFollowingCnt" className='followCnt'></span>
                                            </span> 
                                            <span id="myFollowerInfo" className='followWrap' onClick={()=>{showMyFollowList(false)}}>
                                                팔로워 <span id="myFollowerCnt" className='followCnt'></span>
                                            </span>    
                                        </div>
                                    </>
                                :  <div>팔로워 <span id="followerCnt" className='followCnt'></span></div>
                                }
                               </div>
                            </td>
                        </tr>
            ...
            <div id="followCancelScreen" className='promptBoxScreen hide'>
                <div className='followCancelDiv'>
                    ...
                    <div className='followCncl' onClick={()=>{followingCancel(userNo)}}>팔로우 취소</div>
                    <div className='followCnclCncl' onClick={()=>{document.getElementById("followCancelScreen").classList.add("hide")}}>취소</div>
                </div>
