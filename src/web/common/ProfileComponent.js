...
const ProfileComponent = ({ isMine, userNo }) => {
  ...
  useEffect(() => {
    const asyncUseEffect = async function () {
      ...
      if (userNo !== undefined && userNo !== '') {
        jsonObj = await nb_dataFetch(
          '/takeUserProfile?userNo=' + Number(userNo),
          true
        );
        if (jsonObj.isSuccess) {
          ...
          if (
            ...
            setImgPath(
              process.env.REACT_APP_S3_PATH +
                '/' +
                returnObj.data.fileNameVo.profileImgPath +
                '/' +
                encodeURIComponent(returnObj.data.fileNameVo.profileImgName)
            );
          }
          ...
          document.getElementById('followerCnt').innerText =
            jsonObj.followerCnt;
        }
      } else {
        ...
        if (jsonObj.status == 200) {
          ...
          if (
            jsonObj.data.myProfile.profileImgPath !== null &&
            jsonObj.data.myProfile.profileImgName !== null
          ) {
            setImgPath(
              process.env.REACT_APP_S3_PATH +
                '/' +
                jsonObj.data.myProfile.profileImgPath +
                '/' +
                encodeURIComponent(jsonObj.data.myProfile.profileImgName)
            );
          }
        }
        setMyFollowing(jsonObj.data.followingProfile);
        setMyFollower(jsonObj.data.followerProfile);
        document.getElementById('myFollowerCnt').innerText =
          jsonObj.data.followerCnt;
        document.getElementById('myFollowingCnt').innerText =
          jsonObj.data.followingCnt;
      }
    };
    asyncUseEffect();
  }, []);
    ...
    if (event.target.files[0] !== undefined) {
      ...
      if (returnObj.status == 200) {
        setImgPath(
          process.env.REACT_APP_S3_PATH +
            '/' +
            returnObj.data.fileNameVo.path +
            '/' +
            encodeURIComponent(returnObj.data.fileNameVo.name)
        );
  ...
  const nicknameChange = async (event) => {
    ...
    if (!regex.test(nickval)) {
      await nb_fadeInOutB(
        '닉네임은 공백 없이 한글, 영어, 숫자 조합 12글자까지 가능합니다.',
        2000
      );
      return false;
    } else {
      const jsonReq = { nickname: nickval };
      let jsonObj = await nb_putRequest(
                    ...
                    {isMine ? (
                      <>
                        <span
                          ...
                          onClick={() => {
                            document
                              .getElementById('myNickNameWrap')
                              .classList.add('hide');
                            document
                              .getElementById('nickChngWrap')
                              .classList.remove('hide');
                            document.getElementById('nickname').focus();
                          }}
                        ></span>
                        <sup className='nicknameChngToolTip'>
                          닉네임 수정하기
                        </sup>
                      </>
                    ) : (
                      <>
                        ...
                        <span
                          ...
                          onClick={() => {
                            document
                              .getElementById('followCancelScreen')
                              .classList.remove('hide');
                          }}
                        ></span>
                      </>
                  ...
                  {isMine ? (
                    <>
                      <div>
                        ...
                        >
                          팔로워{' '}
                          <span id='myFollowerCnt' className='followCnt'></span>
                        </span>
                      </div>
                    </>
                  ) : (
                    <div>
                      팔로워{' '}
                      <span id='followerCnt' className='followCnt'></span>
                    </div>
                  )}
                </div>
              </td>
      ...
      {isMine ? (
        ...
        <div id='followCancelScreen' className='promptBoxScreen hide'>
          <div className='followCancelDiv'>
            ...
            <div className='marginTenAuto'>
              {nickname}님의 팔로우를 취소하시겠어요?
            </div>
            <div
