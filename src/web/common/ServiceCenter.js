...
const ServiceCenter = ({ myNickName }) => {
  ...
  const registerError = async () => {
    ...
    formData.append('errType', await cs_toCsErrType(window.errType));
  ...
  const myErrList = myErrReport.map((errMap, idx) => {
    let errType = '1:1 문의';
    if (errMap.errType === 'MathContents') {
      errType = '문제 오류';
    } else if (errMap.errType === 'MathResource') {
      errType = '컨텐츠 오류';
    } else if (errMap.errType === 'MathDocs') {
      errType = '학습지 오류';
    } else if (errMap.errType === 'MathEditor') {
      errType = '수식편집기 오류';
    } else if (errMap.errType === 'HwpConvert') {
      errType = '파일변환 오류';
    }
    ...
    <>
      <div id='serviceCenter' className='blindBox hide'>
        <div className='serviceCenterRootDiv'>
          <div id='serviceCenterRootTitle' className='serviceCenterRootTitle'>
            {myNickName}님, 무엇을 도와 드릴까요?
          </div>
          <div className='serviceCenterTabMenu'>
            <span
              id='servicePolicyTab'
              className='serviceCenterTab active'
              onClick={(event) => {
                serviceCenterMenuClick(event, 'servicePolicy');
              }}>
              운영정책
            </span>
            <span
              id='serviceQuestionTab'
              className='serviceCenterTab'
              onClick={(event) => {
                serviceCenterMenuClick(event, 'serviceQuestion');
              }}>
              1:1 문의
            </span>
            <span
              className='serviceCenterTab'
              onClick={(event) => {
                serviceCenterMenuClick(event, 'myQnA');
              }}>
              나의 문의내역
            </span>
          </div>
          <div
            ...
            onClick={() => {
              document.getElementById('serviceCenter').classList.add('hide');
          ...
          <div>
            ...
            <div id='serviceQuestion' className='serviceCenterMenuCon hide'>
              ...
              <div className='alignCenter'>
                <form method='post' id='oneToOneQuestion' encType='multipart/form-data'>
                  ...
                  <div>
                    <input
                      ...
                      accept='image/*'
                      onChange={(event) => {
                        imgFileChange(event, 'firstImgShow');
                      }}
                    />
                    <img
                      id='firstImgShow'
                      className='imgAddBtn'
                      onClick={() => {
                        document.getElementById('firstImgName').click();
                      }}
                      src={image}
                      alt=''
                    />
                    <input
                      ...
                      accept='image/*'
                      onChange={(event) => {
                        imgFileChange(event, 'secondImgShow');
                      }}
                    />
                    <img
                      id='secondImgShow'
                      className='imgAddBtn'
                      onClick={() => {
                        document.getElementById('secondImgName').click();
                      }}
                      src={image}
                      alt=''
                    />
                    <input
                      ...
                      accept='image/*'
                      onChange={(event) => {
                        imgFileChange(event, 'thirdImgShow');
                      }}
                    />
                    <img
                      id='thirdImgShow'
                      className='imgAddBtn'
                      onClick={() => {
                        document.getElementById('thirdImgName').click();
                      }}
                      src={image}
                      alt=''
                    />
                ...
                <div
                  id='oneToOneQustionBtn'
                  className='errRegisterBtn marginTenAuto'
                  onClick={() => {
                    registerError();
                  }}>
                  접수하기
            ...
            <div id='myQnA' className='serviceCenterMenuCon myQnaMenu hide'>
              ...
              <div id='detailedMyQna' className='detailedMyQna hide'>
                <div
                  className='backBtn'
                  onClick={() => {
                    document.getElementById('detailedMyQna').classList.add('hide');
                    document.getElementById('myQnAList').classList.remove('hide');
                    document.getElementById('replyContentsDiv').classList.add('hide');
                  }}>
                  &#60; 뒤로가기
                </div>
