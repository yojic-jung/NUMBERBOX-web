...
const MyRepository = () => {
  ...
  useEffect(() => {
    const asyncUseEffect = async function () {
      ...
      let contentsNodeList = returnObj.data.contents;
  ...
  const showDetailConInfo = async (contentsNo, userNo, event) => {
    ...
    let contents;
    contentsList.forEach(function (element, idx) {
      if (element.contentsId === Number(contentsNo)) {
        contents = element;
        return false;
      }
    });
  ...
  const workContentsList = contentsList.map((contentsMap, idx) => {
    ...
    return (
      <div
        id='workContentsDiv'
        className='contentsDiv contentsDivForFilter userSearchPage'
        key={idx}
          ...
          <tbody>
            <tr>
              <td
                ...
                onClick={(event) => {
                  showDetailConInfo(contentsMap.contentsId, contentsMap.userId, event);
                }}>
                ...
                <span
                  className='delBtn'
                  onClick={() => {
                    setDelTargetConNo(contentsMap.contentsNo);
  ...
  const goToMathDocs = async () => {
    ...
    document.title = '나의 저장소문제';
    let jsonData = new Object();
    jsonData.docsGrade = '';
    jsonData.docsTitle = '나의 저장소문제';
        ...
        <div>
          <div className='workList custom'>
            {workContentsList.length !== 0 ? (
              <div>
                <div className='contents-show userSearchPage filterContents' id='contents-show'>
                  {workContentsList}
                </div>
              </div>
            ) : (
