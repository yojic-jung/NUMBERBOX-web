...
const quesAnsTabList = [
  { id: 'quesTab', tabName: '문제 입력', className: 'checkedTap' },
  { id: 'ansSolTab', tabName: '해설 및 정답', className: '' },
];
...
const FormulaEditor = ({ contentsNo, contentsClassify, isTransModify }) => {
  ...
  const removeAddedEvent = () => {
    ...
    if (contentsNo !== undefined) {
      document.getElementById('outerFormulaEditor').removeEventListener('scroll', topMenuFixed);
    }
  ...
  useEffect(() => {
    const asyncUseEffect = async function () {
      ...
      window.shortCutKeyHigh1 = jsonObj.data['High1'];
      window.shortCutKeyEtc = jsonObj.data['Etc'];
      window.shortCutKeyEtc2 = jsonObj.data['Etc2'];

      if (contentsNo !== undefined) {
        document.getElementById('outerFormulaEditor').addEventListener('scroll', topMenuFixed);
      } else {
  ...
  const contentsValidation = async function () {
    ...
    let isFirNoExist =
      (!(document.getElementById('firNoFormulaEditor').innerText.length === 1 && document.getElementById('firNoFormulaEditor').innerText === '\n') &&
        document.getElementById('firNoFormulaEditor').innerText.length > 0) ||
      document.getElementById('firNoFormulaEditor').querySelector('img') !== null;
    let isSecNoExist =
      (!(document.getElementById('secNoFormulaEditor').innerText.length === 1 && document.getElementById('secNoFormulaEditor').innerText === '\n') &&
        document.getElementById('secNoFormulaEditor').innerText.length > 0) ||
      document.getElementById('secNoFormulaEditor').querySelector('img') !== null;
    let isThrNoExist =
      (!(document.getElementById('thrNoFormulaEditor').innerText.length === 1 && document.getElementById('thrNoFormulaEditor').innerText === '\n') &&
        document.getElementById('thrNoFormulaEditor').innerText.length > 0) ||
      document.getElementById('thrNoFormulaEditor').querySelector('img') !== null;
    let isFourNoExist =
      (!(document.getElementById('fourNoFormulaEditor').innerText.length === 1 && document.getElementById('fourNoFormulaEditor').innerText === '\n') &&
        document.getElementById('fourNoFormulaEditor').innerText.length > 0) ||
      document.getElementById('fourNoFormulaEditor').querySelector('img') !== null;
    let isFifNoExist =
      (!(document.getElementById('fifNoFormulaEditor').innerText.length === 1 && document.getElementById('fifNoFormulaEditor').innerText === '\n') &&
        document.getElementById('fifNoFormulaEditor').innerText.length > 0) ||
      document.getElementById('fifNoFormulaEditor').querySelector('img') !== null;
  ...
  const formulaConvert = async (event) => {
    ...
    if (evIdName == 'multi-answer') {
      //객관식 정답 선택 이벤트의 경우 단축키 이벤트 없이 진행
        ...
        <div className='twoFlexLayout'>
            ...
            <div id='contents-show' className='contents-show'>
              ...
              <div id='ques-show'>
                ...
                <div id='multi-show' className='multi-show'>
                  <div id='firDiv' className='firDiv hide'>
                    <span className='multiChoiceNo'>&#9312;</span>
                    <span id='firNoShow' dangerouslySetInnerHTML={{ __html: firNo }}></span>
                  </div>
                  <div id='secDiv' className='secDiv hide'>
                    <span className='multiChoiceNo'>&#9313;</span>
                    <span id='secNoShow' dangerouslySetInnerHTML={{ __html: secNo }}></span>
                  </div>
                  <div id='thrDiv' className='thrDiv hide'>
                    <span className='multiChoiceNo'>&#9314;</span>
                    <span id='thrNoShow' dangerouslySetInnerHTML={{ __html: thrNo }}></span>
                  </div>
                  <div id='fourDiv' className='fourDiv hide'>
                    <span className='multiChoiceNo'>&#9315;</span>
                    <span id='fourNoShow' dangerouslySetInnerHTML={{ __html: fourNo }}></span>
                  </div>
                  <div id='fifDiv' className='fifDiv hide'>
                    <span className='multiChoiceNo'>&#9316;</span>
                    <span id='fifNoShow' dangerouslySetInnerHTML={{ __html: fifNo }}></span>
                  </div>
                </div>
              </div>
          ...
          <div className='right'>
            ...
            <div id='topShortkeyDiv'>
              ...
              {isFetchShotCutKey && <FormulaShortCutKey compId='shortKeyBoard' keyName='Main' parentShortCutKey={shortCutKey} parentMethod={showFormulaEditor} />}
              {isFetchShotCutKey && <FormulaShortCutKey compId='shortKeyBoardHigh' keyName='High1' parentShortCutKey={shortCutKey} parentMethod={showFormulaEditor} />}
              {isFetchShotCutKey && <FormulaShortCutKey compId='shortKeyBoardEtc' keyName='Etc' parentShortCutKey={shortCutKey} parentMethod={showFormulaEditor} />}
              {isFetchShotCutKey && <FormulaShortCutKey compId='shortKeyBoardEtc2' keyName='Etc2' parentShortCutKey={shortCutKey} parentMethod={showFormulaEditor} />}
            </div>
            <div>
            ...
            <div
              id='contentsFormulaEditor'
              className='contentsFormulaEditor contentEditClass onlyEdit'
              contentEditable='true'
              spellCheck={false}
              placeholder='문제를 입력해주세요...'
              onKeyDown={(event) => {
                reg_preventKeyEvent(event, isMyContents);
                copyPreventEv(event);
              }}
              onKeyUp={(event) => {
                formulaConvert(event, shortCutKeyList);
                reg_dressSelectionBackColor();
                reg_tbCellKeyUp(event);
                reg_nbComplie(event);
                nb_base64ImgRegisterToS3(event);
                reg_convertFigureTagRemove('contentsFormulaEditor');
              }}
              onClick={() => {
                reg_dressYellowBox();
              }}
            ...
            <div
              id='solutionFormulaEditor'
              className='solutionFormulaEditor contentEditClass onlyEdit hide'
              contentEditable='true'
              spellCheck={false}
              ...
              onKeyUp={(event) => {
                ...
                reg_nbComplie(event);
                nb_base64ImgRegisterToS3(event);
                reg_convertFigureTagRemove('solutionFormulaEditor');
              }}
              onClick={() => reg_dressYellowBox()}
              onMouseDown={() => {
                reg_selectCheck();
              }}
              onPaste={(event) => {
            ...
            <div id='contentsOptBox' className='contentsOptBox marginTen'>
              <div className='mini-title marginBox hide'>
                <span id='cusConUpldBtn' className='uploadBtn'>
                  문제 이미지 첨부
                </span>
              ...
              <div id='multiChoiceBox' className='multiChoiceBox'>
                <input
                  id='multiChoiceImageFile'
                  className='hide'
                  type='file'
                  accept='image/*'
                  onChange={(event) => {
                    nb_extensionCheck2(event);
                    multiChoiceImageFile(event);
                  }}
                />
                <div className='multiChoiceWrap'>
                  <div
                    id='firNoFormulaEditor'
                    contentEditable='true'
                    spellCheck={false}
                    className='multiChoiceView contentEditClass onlyEdit'
                    onKeyDown={(event) => {
                      reg_preventKeyEvent(event, isMyContents);
                      copyPreventEv(event);
                    }}
                    onKeyUp={(event) => {
                      ...
                      reg_nbComplie(event);
                      nb_base64ImgRegisterToS3(event);
                      reg_convertFigureTagRemove('firNoFormulaEditor');
                    }}
                    onClick={() => reg_dressYellowBox()}
                    onMouseDown={() => {
                      reg_selectCheck();
                    }}
                    onPaste={(event) => reg_tbPastePrevent(event)}
                    onCopy={(event) => {
                      reg_imageCopy(event, true);
                    }}
                    onCut={(event) => {
                  ...
                  <div
                    className='multiChoiceImgAddBtn'
                    onClick={() => {
                      multiChoiceImgAdd('firNoFormulaEditor');
                    }}></div>
                </div>
                <div className='multiChoiceWrap'>
                  <div
                    id='secNoFormulaEditor'
                    contentEditable='true'
                    spellCheck={false}
                    className='multiChoiceView contentEditClass onlyEdit'
                    onKeyDown={(event) => {
                    ...
                    onKeyUp={(event) => {
                      ...
                      reg_nbComplie(event);
                      nb_base64ImgRegisterToS3(event);
                      reg_convertFigureTagRemove('secNoFormulaEditor');
                    }}
                    onClick={() => reg_dressYellowBox()}
                    onMouseDown={() => {
                      reg_selectCheck();
                  ...
                  <div
                    className='multiChoiceImgAddBtn'
                    onClick={() => {
                      multiChoiceImgAdd('secNoFormulaEditor');
                    }}></div>
                </div>
                <div className='multiChoiceWrap'>
                  <div
                    id='thrNoFormulaEditor'
                    contentEditable='true'
                    spellCheck={false}
                    className='multiChoiceView contentEditClass onlyEdit'
                    onKeyDown={(event) => {
                    ...
                    onKeyUp={(event) => {
                      ...
                      reg_nbComplie(event);
                      nb_base64ImgRegisterToS3(event);
                      reg_convertFigureTagRemove('thrNoFormulaEditor');
                    }}
                    onClick={() => reg_dressYellowBox()}
                    onMouseDown={() => {
                      reg_selectCheck();
                  ...
                  <div
                    className='multiChoiceImgAddBtn'
                    onClick={() => {
                      multiChoiceImgAdd('thrNoFormulaEditor');
                    }}></div>
                </div>
                <div className='multiChoiceWrap'>
                  <div
                    id='fourNoFormulaEditor'
                    contentEditable='true'
                    spellCheck={false}
                    className='multiChoiceView contentEditClass onlyEdit'
                    onKeyDown={(event) => {
                    ...
                    onKeyUp={(event) => {
                      ...
                      reg_nbComplie(event);
                      nb_base64ImgRegisterToS3(event);
                      reg_convertFigureTagRemove('fourNoFormulaEditor');
                    }}
                    onClick={() => reg_dressYellowBox()}
                    onMouseDown={() => {
                      reg_selectCheck();
                  ...
                  <div
                    className='multiChoiceImgAddBtn'
                    onClick={() => {
                      multiChoiceImgAdd('fourNoFormulaEditor');
                    }}></div>
                </div>
                <div className='multiChoiceWrap'>
                  <div
                    id='fifNoFormulaEditor'
                    contentEditable='true'
                    spellCheck={false}
                    className='multiChoiceView contentEditClass onlyEdit'
                    onKeyDown={(event) => {
                    ...
                    onKeyUp={(event) => {
                      ...
                      reg_nbComplie(event);
                      nb_base64ImgRegisterToS3(event);
                      reg_convertFigureTagRemove('fifNoFormulaEditor');
                    }}
                    onClick={() => reg_dressYellowBox()}
                    onMouseDown={() => {
                      reg_selectCheck();
                  ...
                  <div
                    className='multiChoiceImgAddBtn'
                    onClick={() => {
                      multiChoiceImgAdd('fifNoFormulaEditor');
                    }}></div>
                </div>
                <div className='hide'>
                  &#9312; <textarea className='marginFive' id='firNo' name='firNo' defaultValue={firNo}></textarea>
                  <br />
                  &#9313; <textarea className='marginFive' id='secNo' name='secNo' defaultValue={secNo}></textarea>
                  <br />
                  &#9314; <textarea className='marginFive' id='thrNo' name='thrNo' defaultValue={thrNo}></textarea>
                  <br />
                  &#9315; <textarea className='marginFive' id='fourNo' name='fourNo' defaultValue={fourNo}></textarea>
                  <br />
                  &#9316; <textarea className='marginFive' id='fifNo' name='fifNo' defaultValue={fifNo}></textarea>
                  <br />
                </div>
              </div>
            </div>
            ...
            <div id='ansSolOptBox' className='ansSolOptBox marginTen hide'>
              <div className='mini-title marginBox hide'>
                <span id='cusSolUpldBtn' className='uploadBtn'>
                  해설 이미지 첨부
                </span>
                <span className='descBox'>이미지 삭제를 원하는 경우 이미지를 더블 클릭해주세요.</span>
              ...
              <div>
                ...
                <div
                  id='answerFormulaEditor'
                  className='answerFormulaEditor contentEditClass onlyEdit'
                  contentEditable='true'
                  spellCheck={false}
                  onKeyDown={(event) => {
                    reg_preventKeyEvent(event, isMyContents);
                  ...
                  onPaste={(event) => reg_tbPastePrevent(event)}
                  onDrop={(event) => event.preventDefault()}></div>
                ...
                <div>
                  <input
                    ...
                    name='choiceAnswer'
                    id='multiAns1'
                    className='multiAnsInput hide'
                    value='&#9312;'
                    onChange={(event) => {
                      getCheckedVal(event);
                    }}
                  />
                  <label className='circleBox' htmlFor='multiAns1'>
                    &#9312;
                  </label>
                  ...
                  />
                  <label className='circleBox' htmlFor='multiAns2'>
                    &#9313;
                  </label>
                  ...
                  />
                  <label className='circleBox' htmlFor='multiAns3'>
                    &#9314;
                  </label>
                  ...
                  />
                  <label className='circleBox' htmlFor='multiAns4'>
                    &#9315;
                  </label>
                  ...
                  />
                  <label className='circleBox' htmlFor='multiAns5'>
                    &#9316;
                  </label>
                </div>
              </div>
