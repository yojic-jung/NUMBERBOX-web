...
import {
  reg_getMappingShortCutKeyClk,
  reg_writeDisableDom,
  reg_dressYellowBox,
  reg_reGenerFormulBugFix,
  reg_undoStackByClick,
  reg_undoArrPop,
  reg_addBrInLastPosition,
  reg_nbFormulaConvert,
...
const FormulaShortCutKey = ({ compId, keyName, parentShortCutKey, parentMethod }) => {
  ...
  const addFormulaKey = async (event) => {
    ...
    if (
      !(
        focusId === 'contentsFormulaEditor' ||
        focusId === 'solutionFormulaEditor' ||
        focusId === 'firNoFormulaEditor' ||
        focusId === 'secNoFormulaEditor' ||
        focusId === 'thrNoFormulaEditor' ||
        focusId === 'fourNoFormulaEditor' ||
        focusId === 'fifNoFormulaEditor' ||
        focusId === 'answerFormulaEditor' ||
        focusId === 'myHwpContents' ||
        focusId.indexOf('contentsFormulaEditor') > -1 ||
        focusId.indexOf('solutionFormulaEditor') > -1 ||
        focusId.indexOf('firNoFormulaEditor') > -1 ||
        focusId.indexOf('secNoFormulaEditor') > -1 ||
        focusId.indexOf('thrNoFormulaEditor') > -1 ||
        focusId.indexOf('fourNoFormulaEditor') > -1 ||
        focusId.indexOf('fifNoFormulaEditor') > -1 ||
        focusId.indexOf('answerFormulaEditor') > -1
      )
    ) {
  ...
  const shortCutKeyList = parentKeyList.map((keyLabel, idx) => {
    ...
    return (
      <span key={idx}>
        <button
          type='button'
          className={'keySpan' + arrowKeyClass}
          id={formulaBtnId}
          title={keyLabel.formulName}
          data-formula-id={keyLabel.id}
          onClick={(event) => addFormulaKey(event)}>
  ...
  if (componentId == 'shortKeyBoard') {
    return (
      <>
        <div type='button' id={compId} className='shortKeyBoard' onMouseDown={(event) => keepFocus(event)}>
          {shortCutKey}
        </div>
        ...
        <div id='etcShortCutRootDiv' className='blindBox hide'>
          <div className='etcShortCutDiv'>
            <div
              className='closeBtn2'
              onClick={() => {
                document.getElementById('etcShortCutRootDiv').classList.add('hide');
              }}>
              X
            </div>
            ...
            <div className='etcShortCutKeyDiv'>
              <div>
                <span className='keyboardUI'>shift</span>+<span className='keyboardUI'>space</span> : 띄어쓰기 다섯칸
              </div>
              <div>
                <span className='keyboardUI'>alt</span>+<span className='keyboardUI'>→</span> : 수식 단축키 탭 오른쪽으로 이동
              </div>
              <div>
                <span className='keyboardUI'>alt</span>+<span className='keyboardUI'>←</span> : 수식 단축키 탭 왼쪽으로 이동
              </div>
            </div>
          </div>
        </div>
        ...
        <div id='bugUiRootDiv' className='blindBox hide'>
          <div className='bugUiDescDiv'>
            <div
              className='closeBtn2'
              onClick={() => {
                document.getElementById('bugUiRootDiv').classList.add('hide');
              }}>
              X
            </div>
            <div className='bugUiTitle'>
              <span className='keyboardUI'>alt</span>+<span className='keyboardUI'>=</span> 단축키 버그 유의
            </div>
            <div className='bugUiDesc'>
              alt+'=' 키는 인터넷 브라우저 자체 버그로 아래와 같은 현상이 발생할 수 있습니다.
              <br />
            ...
            <div className='bugUiImgWrap'>
              ...
              <div>
                <img src={shortCutButExImg1} className='bugUiImg' alt='' />
              </div>
              <div className='greenText'>정상 화면</div>
              <div>
                <img src={shortCutButExImg2} className='bugUiImg' alt='' />
              </div>
            </div>
          </div>
        </div>
    ...
    return (
      <>
        <div type='button' id={compId} className='shortKeyBoard hide' onMouseDown={(event) => keepFocus(event)}>
          {shortCutKey}
        </div>
        ...
        <div id='bugUiRootDiv' className='blindBox hide'>
          <div className='bugUiDescDiv'>
            ...
            <div className='bugUiDesc'>
              alt+'=' 키는 브라우저 자체 버그로 아래와 같은 현상이 발생할 수 있습니다.
              <br />
              해당 버그 발생시 한번 더 alt+'=' 키를 다시 누르고 글을 입력하면 정상 출력 됩니다.
            </div>
            <div className='bugUiImgWrap'>
