...
const RegisterContentsForImg = ({ contentsNo }) => {
  ...
  const customImgUpld = async (targetId) => {
    let updtImg = false;
    if ((targetId === 'contentsImg' && conImgName !== 'N') || (targetId === 'solutionImg' && solImgName !== 'N')) {
  ...
  const loadFile = async (event, outputId, contentsNo) => {
    //outputId는 출력 dom
      ...
      <div id='registerQuestion'>
                  ...
                  <tbody>
                    <tr>
                      <td className='regContensForImgTbHeaderTd first relative' colSpan={2}>
                        ...
                        <img
                          className='questionMark rightPosition'
                          src={questionMark}
                          alt='도움말'
                          onClick={() => {
                            document.getElementById('regForImgExampleBox').classList.remove('hide');
                          }}
                        />
                      </td>
                    </tr>
                        ...
                        <div className='regAnswerWrap'>
                          ...
                          <div className='answerWrapForImg'>
                            <div
                              ...
                              className='answerFormulaEditor contentEditClass onlyEdit forImg'
                              contentEditable='true'
                              spellCheck={false}
                              onKeyDown={(event) => reg_preventKeyEvent(event)}
                              onKeyUp={(event) => {
                                formulaConvert(event);
                                reg_dressSelectionBackColor();
                                reg_nbComplie(event);
                              }}
                              ...
                              onPaste={(event) => reg_tbPastePrevent(event)}></div>
                    ...
                    <tr>
                      <td className='regContensForImgTbContents'>
                        ...
                        <div className='alignCenter'>
                          <img
                            id='conImgOutput'
                            className='regContentsImg'
                            src={imgPlus2}
                            alt='문제등록'
                            onClick={() => {
                              customImgUpld('contentsImg');
                            }}
                          />
                        </div>
                        <input
                          id='contentsImg'
                          name='contentsImgFile'
                          type='file'
                          accept='image/*'
                          className='hide'
                          onChange={(event) => {
                            nb_extensionCheck2(event, 'conImgOutput', contentsNo);
                            loadFile(event, 'conImgOutput', contentsNo);
                            nb_addClass('conImgOutput', 'marginTopTenAuto');
                          }}
                        />
                      </td>
                      <td className='regContensForImgTbSolution relative'>
                        ...
                        <div className='alignCenter'>
                          <img
                            id='solImgOutput'
                            className='regSolutionImg'
                            src={imgPlus2}
                            alt='해설등록'
                            onClick={() => {
                              customImgUpld('solutionImg');
                            }}
                          />
                        </div>
                        <input
                          id='solutionImg'
                          name='solutionImgFile'
                          type='file'
                          accept='image/*'
                          className='hide'
                          onChange={(event) => {
                            nb_extensionCheck2(event, 'solImgOutput', contentsNo);
                            loadFile(event, 'solImgOutput', contentsNo);
                            nb_addClass('solImgOutput', 'marginTopTenAuto');
                          }}
                        />
        ...
        <div id='regForImgExampleBox' className='blindBox hide'>
          <div className='regForImgExampleDiv'>
            <div
              id='regForImgExCloseBtn'
              className='closeBtn'
              onClick={() => {
                document.getElementById('regForImgExampleBox').classList.add('hide');
              }}>
              X
            </div>
            <div className='regForImgExampleDivTitle'>이미지로 등록하여 학습지에 사용하거나 다른 사용자와 공유해보세요!</div>
            <div className='regForImgExampleDivDesc'>
              tip1. 문제 번호와 여백을 제거하고 등록하면 학습지 제작시 다른 문제들과 함께 출력하기 용이합니다.
              <br />
